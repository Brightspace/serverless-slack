...
class Client {
	 ...
	 */
	constructor(auth, payload, callback) {
		this.payload = payload || {};
		this.auth = auth || {};
		this.callbackFn = callback || undefined;
	...
	/**
	 * Allow event handlers to use the callback
	 *
	 * @param {Object} response A response object or string
	 */
	callback(response) {
		if (this.callbackFn) {
			let callback = this.callbackFn;
			this.callbackFn = undefined;
			return Promise.resolve(callback(null, JSON.stringify(response)));
		} else {
			return this.reply(reponse);
		}
